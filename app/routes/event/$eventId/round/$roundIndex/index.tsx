...
import type {GameOutcome, PlayerGame,} from "~/tournament/player-game-model.server";
...
function PlayerGamesTable({playerGames, tournament, roundIndex, attendeesBySlug}: PlayerGamesTableProps) {
  return <table className="round-table">
    <thead>
      <tr>
        <th>Table</th>
        <th>Player</th>
    ...
    <tbody>
      {
        ...
        playerGames.reduce<{ prevTable: number; rows: ReactNode[] }>(
          ({prevTable, rows}, pg) => ({
...
export default function RoundIndexPage() {
  ...
  const firstHalf = sortedGames.slice(0, Math.ceil(sortedGames.length / 2));
  const secondHalf = sortedGames.slice(Math.ceil(sortedGames.length / 2));
